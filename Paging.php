...
class Page{
   ...
   * 键为页码，值为链接
   * $this->page_arr=Array(
   *     [1] => meesageboard.php?page=1
   *     [2] => meesageboard.php?page=2
   *     [3] => meesageboard.php?page=3
   *     ......)
   */
...
$result=$conn->query("SELECT * FROM message") or die($conn->errno);
$numbers=$result->num_rows;


$Demo=new Page($numbers,ceil($numbers/2),isset($_GET['page']) ? $_GET['page'] : 1,2,"");
if($numbers>0)
{
    $start_index = ($Demo->currPage-1)*$Demo->subPages;
    $result1=$conn->query("SELECT * FROM message LIMIT ".$start_index."," .$Demo->subPages);
    while($rows=$result1->fetch_array())
    {
        echo $rows['id'].$rows['title']."<br>";
    }
}
print_r($Demo->getPages());
echo $Demo->showPages(1);
?>