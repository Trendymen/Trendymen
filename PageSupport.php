...
//方法
     ...
     * $ppage_size：每页显示最多行数
     ***********************************************/
    function __construct($ppage_size = 5, $sql)
    {
        $this->mysql = new mysql();
        $this->conn = $this->mysql->conn;
        $this->page_size = $ppage_size;
        $this->start_index = 0;
        $this->sql = $sql;
        $this->current_page = isset($_GET['current_page']) ? $_GET['current_page'] : 1;
        if (isset($this->current_page)) {

            $this->__set("current_page", $this->current_page);

        } else {

            $this->__set("current_page", 1);
    ...
    function standard_navigate()
    {
        ...
        echo "跳到<input type=\"number\" size=\4\" name=\"current_page\" value='" . $this->current_page . "'/>页";
    ...
    function output()
    {
        if ($this->current_records > 0) //如果数据不为空，则组装数据
        {
            for ($i = 0; $i < $this->current_records; $i++) {
                ...
                $content = $this->result[$i]["content"];

                $part = substr($content, 0, 400);
                //循环输出每条数据
                ?>
                <div class="index_side">
                    <div class="index_title"><?php echo $id ?>楼</div>
                    <div class="index_title"><?php echo $title ?></div>
                    <div class="index_content"><?php echo $part ?></div>
                </div>
                <?php
            }
