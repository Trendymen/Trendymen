    ...
    E.createMenu(function (check) {
        ...
        $.each(configEmotions, function (k, emotion) {
            ...
            if (typeof data === 'string') {
                ...
                // 数组，即 data.js 直接就是表情包数据
                insertEmotionImgs(data, $tabContent);
            } else {
                // 其他情况，data格式不对
                E.error('data.json 数据格式错误，请修改为正确格式，参考文档：' + E.docsite);
                return;
            }
        });
...
_e(function (E, $) {
    ...
    UploadFile.fn.render = function () {
        ...
        var $form = $('<form enctype="multipart/form-data.js" method="post" action="' + uploadUrl + '" target="' + iframeId + '"></form>');
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        $txt.on('paste', function (e) {
            ...
            if (items) {
                // -------- chrome 可以用 data.js.items 取出图片 -----
                E.log('通过 data.js.items 得到了数据');
                ...
                // -------- 非 chrome 不能用 data.js.items 取图片 -----

                E.log('未从 data.js.items 得到数据，使用检测粘贴图片的方式');
