...
function preparePlaceholder() {
    ...
    placeholder.setAttribute("alt", "my image images");
...
function disaplayAjaxLoading(element) {
    var num = element.children.length;
    for (var i = 0; i < num; i++) {
...
function submitFormWithAjax(form, target) {
    ...
    disaplayAjaxLoading(target);
    var dataarr = [];
    var element;
    for (var i = 0; i < form.elements.length; i++) {
        element = form.elements[i];
        dataarr[i] = element.name + "=" + encodeURIComponent(element.value);
    }
    var data = dataarr.join("&");
    ajax.open("post", form.getAttribute("action"), true);
    ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    ajax.onload = function () {
        window.time=setTimeout(function () { //由于是本地服务器 不设置延迟，不会有加载动画出现
            if (ajax.readyState == 4) {
                var article = ajax.responseText;
                if (article !== null) {
                ...
                target.innerHTML = "<p>" + ajax.statusText + "</p>";
            }
        }, 500);
    };
    ajax.send(data);
